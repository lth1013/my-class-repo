...
// To update an object, the numerical id string must be wrapped with ObjectID()
app.put('/update', (req, res) => {
  // Use updateOne() to delete one object
  const bookId = new ObjectId(req.body.id);
  db.collection('bookCollection').updateOne(
    ...
    console.log(results);
    res.send(
      results.modifiedCount ? 'Document Updated' : 'No document found!'
    );
  })
  .catch(err => {
    if (err) throw err;
